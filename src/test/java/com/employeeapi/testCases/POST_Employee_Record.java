...
public class POST_Employee_Record extends TestBase {
	...
	void createEmployee() throws InterruptedException {
		...
		logger.info("********Started Add_Employee_Record********");
		...
		response = given().request(Method.POST, "/create");
	...
	void checkResponseBody() {
		...
		Assert.assertEquals(jsonPath.get("message"), "Successfully! Record has been added.");
		Assert.assertTrue(jsonPath.get("data").toString().matches("\\{id=\\d{4}\\}"));		
...
//	}
	...
	void tearDown() {
		...
		logger.info("********Finished Add_Employee_Record********");
